...
int open_port(void) {
	...
	//printf("Checking communication port : /dev/com4\n",s);
	fd = open("/dev/ttyUSB0", O_RDWR | O_NOCTTY | O_NDELAY); // khusus linux
	//fd = open("/dev/com4", O_RDWR | O_NOCTTY | O_NDELAY); // khusus windows
	if(fd == -1) {
